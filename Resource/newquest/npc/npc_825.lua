local sq = the_script();
sq:add(ScriptQuest.NPC_VISIT, 825);
...
--访问任务1级环时
	if type == 1 then
	  if qss < 4 then 
			...
			menu = string.gsub(menu_825_1, "#place#", visit_npc_list110[r][1]);	
			menu = 	string.gsub(menu, "#npc#", visit_npc_list110[r][2]);
			id = visit_npc_list110[r][2];
		end
		if qss >= 4 then 
			r = math.random(1, table.getn(visit_npc_list1102));
			menu = string.gsub(menu_825_1, "#place#", visit_npc_list1102[r][1]);	
			menu = 	string.gsub(menu, "#npc#", visit_npc_list1102[r][2]);	
			id = visit_npc_list1102[r][2];
		end
...
--杀怪任务1级环时
	if type == 2 then
		...
		if qss < 4 then 
			r = math.random(1, table.getn(kill_npc_list110));
			menu = string.gsub(menu_825_2, "#name#", kill_npc_list110[r][1]);
			menu = 	string.gsub(menu, "#npc#", kill_npc_list110[r][2]);
			id = kill_npc_list110[r][2]; 
		end
		
		if qss >= 4 then 
			r = math.random(1, table.getn(kill_npc_list1102));
			menu = string.gsub(menu_825_2, "#name#", kill_npc_list1102[r][1]);
			menu = 	string.gsub(menu, "#npc#", kill_npc_list1102[r][2]);
			id = kill_npc_list1102[r][2]; 
		end
...
--收集物品任务	
	if type == 3 then
		if qss < 4 then 
			...
			menu = string.gsub(menu_825_3, "#name#", get_item_list110[r][1]);	
			menu = 	string.gsub(menu, "#id#", get_item_list110[r][2]);
			id = get_item_list110[r][2];
		end
		if qss >= 4 then 
			r = math.random(1, table.getn(get_item_list1102));
			menu = string.gsub(menu_825_3, "#name#", get_item_list1102[r][1]);	
			menu = 	string.gsub(menu, "#id#", get_item_list1102[r][2]);
			id = get_item_list1102[r][2];
		end
...
--杀怪收集物品任务
	if type == 4 then
		if qss < 4 then 
			...
			menu = string.gsub(menu_825_13, "#name#", kill_get_list[r][1]);	
		...
		if qss >= 4 then 
			...
			menu = string.gsub(menu_825_13, "#name#", kill_get_list2[r][1]);	
...
--收集装备任务
	if type == 5 then
		if qss < 4 then 
			...
			menu = string.gsub(menu_825_15, "#name#", get_weapon_list110[r][1]);	
			menu = string.gsub(menu, "#id#", get_weapon_list110[r][2]);
			menu = string.gsub(menu, "#color#", get_weapon_list110[r][3]);
			id = get_weapon_list110[r][2];
			color = get_weapon_list110[r][3];
		end
		if qss >= 4 then 
			r = math.random(1, table.getn(get_weapon_list1102));
			menu = string.gsub(menu_825_15, "#name#", get_weapon_list1102[r][1]);	
			menu = string.gsub(menu, "#id#", get_weapon_list1102[r][2]);
			menu = string.gsub(menu, "#color#", get_weapon_list1102[r][3]);
			id = get_weapon_list1102[r][2];
			color = get_weapon_list1102[r][3];
		end
...
--任务等级判断
	...
	if level ~= 1 and level ~= 110 and level ~= 0 then 
		return 0;
	end
...
--等级判断
	...
	local s = v:get("state");
	if (player.charbase.level < 110 or player.charbase.level >= 120) and s ~= 1 and s ~= -3 then
			return 0;
	end
...
--环数到上限，设置个人变量	
	...
	if not v then
		v = Vars(21102);
		local type, id = select_quest110(npc,v);
...
--物品收集交任务
	if s == 1 and type == 3 then
			...
			if qss < 4 then 
				menu = string.gsub(menu_825_8, "#name#", get_item_list110[r][1]);	
				menu = 	string.gsub(menu, "#id#", get_item_list110[r][2]);
			end 
			if qss >= 4 then 
				menu = string.gsub(menu_825_8, "#name#", get_item_list1102[r][1]);	
				menu = 	string.gsub(menu, "#id#", get_item_list1102[r][2]);
			end 
			show_dialog(npc, menu);
		return 1;	
...
--杀怪收集物品交任务	
	if s == -3 and type == 4 then
			local r=v:get("questkillobject");
			if qss < 4 then
				menu = string.gsub(menu_825_31, "#name#", kill_get_list[r][1]);	
				menu = 	string.gsub(menu, "#kill#", kill_get_list[r][2]);
				menu = 	string.gsub(menu, "#item#", kill_get_list[r][3]);
			end 
			if qss >= 4 then
				menu = string.gsub(menu_825_31, "#name#", kill_get_list2[r][1]);	
				menu = 	string.gsub(menu, "#kill#", kill_get_list2[r][2]);
				menu = 	string.gsub(menu, "#item#", kill_get_list2[r][3]);
			end 
			show_dialog(npc, menu);
...
--收集装备交任务对话
	...
	if s == 1 and type == 5 and (weapon ~= 0 or weapon2 ~= 0) then
		if qss < 4 then	
			...
			menu = string.gsub(menu_825_35, "#name#", get_weapon_list110[r][1]);	
			menu = 	string.gsub(menu, "#id#", get_weapon_list110[r][2]);
			menu = 	string.gsub(menu, "#color#", get_weapon_list110[r][3]);
			show_dialog(npc, menu);
			return 1;
		...
		if qss >= 4 then	
			local r=v:get("weaponrequire");
			menu = string.gsub(menu_825_35, "#name#", get_weapon_list1102[r][1]);	
			menu = 	string.gsub(menu, "#id#", get_weapon_list1102[r][2]);
			menu = 	string.gsub(menu, "#color#", get_weapon_list1102[r][3]);
			show_dialog(npc, menu);
			return 1;
...
--收集装备接任务对话
	if s == 1 and type == 5 then
			...
			if qss >= 4 then
				menu = string.gsub(menu_825_45, "#name#", get_weapon_list1102[r][1]);	
				menu = 	string.gsub(menu, "#id#", get_weapon_list1102[r][2]);
				menu = 	string.gsub(menu, "#color#", get_weapon_list1102[r][3]);
			end
			show_dialog(npc, menu);
...
--杀BOSS接任务对话
	...
	--print("判断通过！！");
			...
			if qss >= 4 then
				menu = string.gsub(menu_825_7, "#name#", kill_boss_list2[r][1]);	
				menu = 	string.gsub(menu, "#id#", kill_boss_list2[r][2]);
			end
			show_dialog(npc, menu);
		return 1;	
...
--print("30~40找人任务");
		...
		if( v:get("state") == 2) and flag == 1 and type == 1 then
			...
			if qss < 4 then 
				...
				menu = 	string.gsub(menu, "#npc#", visit_npc_list110[r][2]);
				show_dialog(npc, menu);
				local xxxx="把信带给#place#然后告诉环式任务使者(110)";
...
--杀怪任务1级接任务判断
	  if( v:get("state") == 2) and flag == 2 then
			...
			if qss < 4 then 
				menu = string.gsub(menu_825_6, "#name#", kill_npc_list110[r][1]);
				menu = 	string.gsub(menu, "#npc#", kill_npc_list110[r][2]);
				local xxxx="攻击#name#15只然后告诉环式任务使者(110)";
...
--收集物品1级接任务判断	
		if( v:get("state") == 2) and flag == 3 then
			...
			if qss < 4 then 
				menu = string.gsub(menu_825_4, "#name#", get_item_list110[r][1]);	
				menu = 	string.gsub(menu, "#id#", get_item_list110[r][2]);
				local xxxx="去收集#name#3个交给环式任务使者(110)";
...
--收集装备接任务判断  
		if( v:get("state") == 2) and type == 5 and flag == 15 then
		  ...
		  if qss < 4 then 
				...
				menu = 	string.gsub(menu, "#id#", get_weapon_list110[r][2]);
				menu = 	string.gsub(menu, "#color#", get_weapon_list110[r][3]);
				local xxxx="收集一个#name#交给环式任务使者(110)";
...
--杀BOSS任务接任务判断
		if( v:get("state") == 2) and type == 6 and flag == 17 then
			...
			if qss < 4 then 
				...
				menu = string.gsub(menu, "#id#", kill_boss_list[r][2]);
				show_dialog(npc, menu);
				local xxxx="去杀#name#然后告诉环式任务使者(110)";
