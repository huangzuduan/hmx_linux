...
{
	memcpy(this->m_arrHost, other.m_arrHost, sizeof(m_arrHost));
	memcpy(this->m_arrUser, other.m_arrUser, sizeof(m_arrUser));
	memcpy(this->m_arrPasswd, other.m_arrPasswd, sizeof(m_arrPasswd));
	memcpy(this->m_arrDBName, other.m_arrDBName, sizeof(m_arrDBName));
...
{
	strncpy(m_arrHost, ip, sizeof(m_arrHost));
	strncpy(m_arrUser, username, sizeof(m_arrUser));
	strncpy(m_arrPasswd, passwd, sizeof(m_arrPasswd));
	strncpy(m_arrDBName, dbname, sizeof(m_arrDBName));
	...
	while (temp->name)
	{
		if (strlen(temp->name) != 0)
		{
			...
			case DB_BIN2:
			{
				...
				if (row[i])
				{
					uint32 bin2size = *((int32*)row[i]) + sizeof(int32);
...
int32 DbMysql::ExecInsert(const char *tableName,const dbCol *column,const char *data)
{
	...
	while(temp->name) 
	{
		...
		{       
			...
			case DB_STR:
				{
					uint32 len = strlen((const char *)(data+offset));
...
int32 DbMysql::ExecUpdate(const char *tableName,const dbCol *column,const char *data,const char *where)
{
	...
	while(temp->name)
	{
		...
		if (len > 0 && strcmp(temp->name, "`CHARID`") != 0)
		{
			...
			case DB_STR:
				{
					uint32 _len=strlen((const char *)(data+offset));
