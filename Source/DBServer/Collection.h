...
protected:
	...
	typedef typename std::multimap<int64, TData*> KeyValueMap;
	typedef typename std::multimap<int64, TData*>::iterator KeyValueMapIter;
...
public:
	...
	inline TData* getMapOne(QWORD multiid,QWORD keyid,DWORD keyoffset = 0)
	{
		...
		for (typename std::vector<TData*>::iterator it = vecData.begin(); it != vecData.end(); ++it)
		{
			QWORD keyidObj = 0;
	...
	{
		typename std::vector<TData*>::iterator it = vecData.begin();
	...
	{
		nowTime = _nowtime;
		boost::mutex::scoped_lock cLock(mutex);
