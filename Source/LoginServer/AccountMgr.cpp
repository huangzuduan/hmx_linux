...
bool AccountMgr::add(Account* obj)
{
	...
	return zEntryMgr< zEntryID, zEntryName >::addEntry(obj);
}

void AccountMgr::remove(Account* obj)
{
	zEntryMgr< zEntryID, zEntryName >::removeEntry(obj);
}

Account* AccountMgr::get(int64 id)
{
	return (Account*)zEntryMgr< zEntryID, zEntryName >::getEntryByID(id);
}

Account* AccountMgr::getByName(const char* name)
{
	return (Account*)zEntryMgr< zEntryID, zEntryName >::getEntryByName(name);
}
...
void AccountMgr::doLoginNamePass(zSession* pSession, const NetMsgSS* pMsg)
{
	...
	if (strncasecmp((const char*)passMd5, account->name, MAX_NAMESIZE) != 0)
	{
		sendRet.accid = 0;
		sendRet.result = 0;
		pSession->sendMsg(&sendRet, sizeof(sendRet));
...
void AccountMgr::doRandRsaAccount(zSession* pSession, const NetMsgSS* pMsg)
{
	...
	int64 maxAccount = GameService::getMe().getAccountMgr().getMaxID();
	...
	if (!GameService::getMe().getAccountMgr().add(account))
	{
		sendRet.result = 2;
		pSession->sendMsg(&sendRet, sizeof(sendRet));
		S_SAFE_DELETE(rsa);
