...
{
	return zEntryMgr< zEntryID, zEntryTempID, zEntryName >::addEntry(user);
}

WorldUser* WorldUserMgr::get(QWORD id)
{
	return (WorldUser*)zEntryMgr< zEntryID, zEntryTempID, zEntryName >::getEntryByID(id);
}

WorldUser* WorldUserMgr::getBySessID(QWORD sessid)
{
	return (WorldUser*)zEntryMgr< zEntryID, zEntryTempID, zEntryName >::getEntryByTempID(sessid);
}

WorldUser* WorldUserMgr::getByName(const char* name)
{
	return (WorldUser*)zEntryMgr< zEntryID, zEntryTempID, zEntryName >::getEntryByName(name);
}

void WorldUserMgr::remove(WorldUser* user)
{
	zEntryMgr< zEntryID, zEntryTempID, zEntryName >::removeEntry(user);
}
...
bool WorldUserMgr::checkmd5(int64 accid, int32 keytime, const char* keymd5)
{
	...
	if (strncasecmp((const char*)makeKeymd5, (const char*)reciveKeymd5,32) != 0)
	{
