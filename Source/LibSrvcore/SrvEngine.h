...
struct zEntry##TempName :public zEntryC##TempName \
{\
...
{
public:
	zSession(NetSocket* _socket):socket(_socket),serivceid(tempid)
	{
		initEncrypt();
	}
...
*
*/
class zSessionMgr : protected zEntryMgr<zEntryID>
{
protected:
...
public:
	...
	{
		return zEntryMgr<zEntryID>::addEntry(conn);
	}

	void remove(zSession* conn)
	{
		zEntryMgr<zEntryID>::removeEntry(conn);
	}

	zSession* get(QWORD id)
	{
		return (zSession*)zEntryMgr<zEntryID>::getEntryByID(id);
	}
	...
	{
		return zEntryMgr<zEntryID>::size();
	}
...
class zServerRegMgr : protected zEntryMgr<zEntryID, zEntryTempID>
{
public:
	zServerRegMgr();
	~zServerRegMgr();
...
public:
	...
	struct Serivce
	{
		...
		int32 id;
		char name[MAX_NAMESIZE + 1];
		char fun[MAX_NAMESIZE + 1];
		int32 remoteid;
		char ip[MAX_NAMESIZE + 1];
		int16 port;
		int16 maxConnects;
		char user[MAX_NAMESIZE + 1];
		char passwd[MAX_NAMESIZE + 1];
		char value[MAX_NAMESIZE + 1];
		int32 broadcast;
	...
	struct Server
	{
		...
		{
			if (strncasecmp(name, "gatewayserver",13) == 0)
			{
				return zSession::FOR_FEP;
			}
			else if (strncasecmp(name, "loginserver",11) == 0)
			{
				return zSession::FOR_LOGIN;
			}
			else if (strncasecmp(name, "worldserver",11) == 0)
			{
				return zSession::FOR_WORLD;
			}
			else if (strncasecmp(name, "sceneserver",11) == 0)
			{
				return zSession::FOR_SCENE;
			}
			else if (strncasecmp(name, "dbserver",8) == 0)
			{
...
public:
	...
	{
		return 1;//(nMin + rand() / (RAND_MAX + 1) * (nMax - nMin));
	}
