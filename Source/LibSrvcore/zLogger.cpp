...
*/
zLogger::zLogger(const char *name)
{
	fp_console = stdout;
	fp_file    = NULL;
...
void zLogger::logva(const zLevel level,const char * pattern,va_list vp)
{
...
#else
		fprintf(fp_console, "%02d:%02d:%02d ", now->tm_hour, now->tm_min, now->tm_sec);
#endif // WIN32	
	}
	if (NULL != fp_file)
	{
...
#else
		fprintf(fp_file, "%02d:%02d:%02d ", now->tm_hour, now->tm_min, now->tm_sec);
#endif // WIN32	
	}
	...
	{
		va_list aq;
		va_copy(aq, vp);// ----两个vfprintf连续使用必须复制一份。
		vfprintf(fp_console,pattern, aq);
		fprintf(fp_console,"\n");
		fflush(fp_console);
	}
	if (NULL != fp_file)
