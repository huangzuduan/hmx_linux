...
bool GameService::init()
{
	...
	{
		H::logger->error("Create Chat World Channel Fail!");
		return false;
	}
	...
	if (!server)
	{
		H::logger->error("Not found serverID=%u serivces.xml", serverID);
		return false;
	}
	...
	for (;it != server->serivces.end();++it)
	{
		...
		if (strncasecmp(info.name,"server",6) == 0)
		{
		...
		else if (strncasecmp(info.name,"client",6) == 0)
		{
		...
		else if (strncasecmp(info.name, "mysqld",6) == 0)
		{
			if(!dbCoon)
			{
				dbCoon = new DbMysql();
				bResult = dbCoon->Open(info.ip,info.port,info.user,info.passwd,info.value);
				if (!bResult)
				{
...
int main(int argc, const char * argv[])
{
	...
	/* 设置缺省参数  */
	H::global["datadir"] = "../../Resource/map/";
	H::global["confdir"] = "../../Resource/";
